...
const reviewLabels=['current','baseline','review','post-merge','pr50-musicality'];
...
if(!reviewLabels.includes(options.label)||![24000,44100,48000].includes(options.sampleRate))throw Error('Expected label='+reviewLabels.join('|')+' and sample-rate=24000|44100|48000');
...
const server=createServer(async(req,res)=>{
  ...
  try {
    ...
    if(url.pathname==='/__music_manifest'){
      const label=url.searchParams.get('label');if(!reviewLabels.includes(label))throw Error('Invalid manifest label');
