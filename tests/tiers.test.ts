...
/**
 * The ladder as the controller walks it, rather than as a pure function.
 *
 * Both of these were review findings, and both were silent: nothing failed, the
 * settings panel reported the right thing, and the picture did not match it.
 */
describe('walking the ladder', () => {
  it('has a rung past every one that gives nothing back', () => {
    // A player who has already turned an effect off by hand should not have
    // the ladder spend a hold "shedding" it. Every rung above one whose
    // derived quality is unchanged must still reach the leanest picture.
    const off: RenderQuality = { ...DEFAULT_QUALITY, bloom: false, pools: false, grade: false };
    const last = derive(off, MAX_RUNG);
    expect(last.shadows).toBe(false);
    expect(last.roll).toBe(false);
    expect(last.renderScale).toBeLessThan(1);

    // And the rungs that do nothing for this preference are detectable as
    // such: their derived quality equals the rung before them.
    const inert: number[] = [];
    for (let r = 1; r <= MAX_RUNG; r++) {
      if (JSON.stringify(derive(off, r)) === JSON.stringify(derive(off, r - 1))) inert.push(r);
    }
    // Grade, floor light and bloom were already off by hand, so their rungs
    // are inert -- which is exactly what the controller has to skip past.
    expect(inert.length).toBeGreaterThan(0);
    expect(inert).not.toContain(MAX_RUNG);
  });

  it('can always shed further than the rung it failed to climb back to', () => {
    // The failed-recovery latch is a floor on climbing back, not a cap on the
    // rung. An earlier version capped the rung itself, so a machine that had
    // failed to recover to rung 4 could never afterwards shed past 5 -- and if
    // the load then grew, the one thing it needed was the one thing it could
    // not do. Expressed here as the property that matters: every rung below
    // the top has somewhere further to go.
    for (let r = 0; r < MAX_RUNG; r++) {
      expect(derive(DEFAULT_QUALITY, r + 1)).not.toEqual(derive(DEFAULT_QUALITY, r));
    }
  });
});
